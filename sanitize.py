...
cursor = cnx.cursor()  
query = "UPDATE event SET redaction=detail, detail='<recipients removed>' where tags='Email';"  
cursor.execute(query)
query = "UPDATE event SET redaction=detail, detail='<attendees removed>' where tags='Calendar';"  
cursor.execute(query)
...
with open(rd) as f:
    for line in f:
        x = line.strip()
        query = "UPDATE event SET title=REPLACE(title, '%s', '<customer>'), redaction='%s' WHERE title LIKE BINARY '%%%s%%';" % (x,x,x)
        #print(query)
        cursor.execute(query)
